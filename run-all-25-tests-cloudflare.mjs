...
async function runAll25TestsInCloudflare() {
  ...
  const wranglerProcess = spawn('wrangler', ['dev', '--local', '--port', '8800'], {
    cwd: workerDir,
    stdio: 'pipe'
  });
  ...
  try {
    ...
    const response = await fetch('http://localhost:8800/test');
...
async function main() {
  ...
  const success = await runAll25TestsInCloudflare();
