...
describe('Neural', () => {
    ...
    describe('Clean Conversation', () => {
        ...
        test('should properly decode object', done => {
            return testContext.connection.neural.cleanConversation("abc123").then(convo => {
                // Test message-specific values were parsed
                expect(convo.id).toEqual("abc123");
                expect(convo.body).toEqual("<img src='cid:1781777f666586677621' /> This is the body");

                // Test conversation-specific values were parsed
                expect(convo.conversation).toEqual("<img src='cid:1781777f666586677621' /> This is the conversation");
        ...
        test('should properly extract all images and return a file', async done => {
            ...
            const convo = await testContext.connection.neural.cleanConversation("abc123");
    ...
    describe('Sentiment Analysis', () => {
        ...
        test('should properly decode object from message', done => {
            return testContext.connection.neural.sentimentAnalysisMessage("abc123").then(convo => {
                const sentBody = testContext.connection.neural._request.mock.calls[0][1];
        ...
        test('should properly decode object from text', done => {
            return testContext.connection.neural.sentimentAnalysisText("This is some text").then(convo => {
                const sentBody = testContext.connection.neural._request.mock.calls[0][1];
    ...
    describe('Extract Signature', () => {
        beforeEach(() => {
            ...
            testContext.connection.neural._request = jest.fn(() => Promise.resolve(serverResponse));
        });

        test('should properly decode both the signature and contact', done => {
            return testContext.connection.neural.extractSignature("abc123").then(sig => {
                expect(sig.signature).toEqual("Nylas Swag\n\nSoftware Engineer\n\n123-456-8901\n\nswag@nylas.com");
                ...
                expect(sig.contacts.links[0].url).toEqual("https://example.com/link.html");
        ...
        test('should properly convert contact in signature to contact object', done => {
            return testContext.connection.neural.extractSignature("abc123").then(sig => {
                ...
                expect(contact.webPages[0].url).toEqual("https://example.com/link.html");
                done();
            });
        })
    });
    ...
    describe('Categorize', () => {
        ...
        test('should properly decode the category object', done => {
            return testContext.connection.neural.categorize("abc123").then(message => {
                const categorizer = message.categorizer;
                expect(categorizer).toBeDefined();
                expect(categorizer.category).toEqual("feed");
                expect(categorizer.categorizedAt).toEqual(new Date("2021-06-24T21:28:09.549Z"));
