...
function connectToNylas(data) {
  return nylasCode(data).then((code) => {
    let params = { 'client_id':     config.nylasClientId,
                   'client_secret': config.nylasClientSecret,
                   'code':          code };
    return nylasToken(params).then((token) => {
      return Promise.resolve(token)
    }).catch((err) => {
      return Promise.reject(new Error(`Could not connect to Nylas. Error: ${err.message}`));
    });
  }).catch((err) => {
    return Promise.reject(new Error(`Could not connect to Nylas. Error: ${err.message}`));
  });
...
function nylasCode(data) {
  ...
  return request.post({ url: connect_uri, json:data}).then((body) => {
    return Promise.resolve(body.code);
  }).catch((err) => {
    return Promise.reject(new Error(`Could not fetch Nylas code: ${err.message}`));
  });
}

function nylasToken(data) {
  let token_uri = config.nylasApi + '/connect/token';
  return request.post({ url: token_uri, json:data}).then((body) => {
    return Promise.resolve(body.access_token);
  }).catch((err) => {
    return Promise.reject(new Error(`Could not fetch Nylas access token: ${err.message}`));
  });
}
