...
const GOOGLE_OAUTH_TOKEN_VALIDATION_URL = 'https://www.googleapis.com/oauth2/v2/tokeninfo'
const GOOGLE_OAUTH_AUTH_URL = 'https://accounts.google.com/o/oauth2/v2/auth'
const GOOGLE_OAUTH_ACCESS_TOKEN_URL = 'https://www.googleapis.com/oauth2/v4/token'

function get_email_from_access_token(google_access_token) {
  let data = { 'access_token': google_access_token,
               'fields':       'email',
  };
  return request.post({url: GOOGLE_OAUTH_TOKEN_VALIDATION_URL, form:data}).then((body) => {
...
router.get('/google', function(req, res, next) {
  ...
  if (!req.session.nylas_access_token) {
    ...
    get_email_from_access_token(google_access_token).then((email) => {
      let google_refresh_token = req.session.google_refresh_token;
      let google_settings = { 'google_client_id': config.googleClientId,
                              'google_client_secret': config.googleClientSecret,
                              'google_refresh_token': google_refresh_token };
      let data= { 'client_id': config.nylasClientId,
                  'name': 'Your Name',
                  'email_address': email,
                  'provider': 'gmail',
...
router.post('/exchange', function(req, res, next) {
  ...
  if (!req.session.nylas_access_token) {
    let json = req.body
    let exchange_settings = { 'username': json.email,
