 ...
 */
export async function sendStringAttachments(
  ...
  if (large) {
    ...
    if (largeFiles.length > 0) {
      const file =
        largeFiles.find((f) => f.filename.includes('pdf')) || largeFiles[0];
    ...
    if (smallFiles.length >= 2) {
      ...
      const textFile =
        smallFiles.find((f) => f.filename.includes('.txt')) || smallFiles[0];
      const imageFile =
        smallFiles.find((f) => f.filename.includes('.jpg')) || smallFiles[1];
