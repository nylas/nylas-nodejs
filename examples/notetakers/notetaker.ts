...
async function inviteNotetaker(): Promise<NylasResponse<Notetaker>> {
  ...
  try {
    ...
    if (!meetingLink) {
      throw new Error(
        'MEETING_LINK environment variable is not set. Please set it with your meeting URL.'
      );
    }
...
async function listNotetakers(): Promise<NylasListResponse<Notetaker>> {
  ...
  try {
    ...
    console.log(
      `Found ${notetakers.data.length} notetakers (ordered by name):`
    );
    for (const notetaker of notetakers.data) {
      console.log(
        `- ${notetaker.name} (ID: ${notetaker.id}, State: ${notetaker.state})`
      );
    }
...
async function getNotetakerMedia(
  notetakerId: string
): Promise<NylasResponse<NotetakerMedia>> {
  ...
  try {
    ...
    if (media.data.recording) {
      ...
      const createdAt =
        'createdAt' in recording
          ? recording.createdAt
          : 'created_at' in recording
            ? (recording as any).created_at
            : undefined;
      const expiresAt =
        'expiresAt' in recording
          ? recording.expiresAt
          : 'expires_at' in recording
            ? (recording as any).expires_at
            : undefined;

      if (createdAt) {
        console.log(
          `Recording Created: ${new Date(createdAt * 1000).toISOString()}`
        );
      }
      if (expiresAt) {
        console.log(
          `Recording Expires: ${new Date(expiresAt * 1000).toISOString()}`
        );
      }
    ...
    if (media.data.transcript) {
      ...
      // Handle both snake_case (API) and camelCase (SDK model) property naming
      const createdAt =
        'createdAt' in transcript
          ? transcript.createdAt
          : 'created_at' in transcript
            ? (transcript as any).created_at
            : undefined;
      const expiresAt =
        'expiresAt' in transcript
          ? transcript.expiresAt
          : 'expires_at' in transcript
            ? (transcript as any).expires_at
            : undefined;

      if (createdAt) {
        console.log(
          `Transcript Created: ${new Date(createdAt * 1000).toISOString()}`
        );
      }
      if (expiresAt) {
        console.log(
          `Transcript Expires: ${new Date(expiresAt * 1000).toISOString()}`
        );
      }
    ...
    if (error instanceof NylasApiError) {
      ...
      console.error(
        `Unexpected error in get_notetaker_media: ${error.message}`
      );
    }
    throw error;
  }
 ...
 */
async function leaveNotetaker(
  notetakerId: string
): Promise<NylasResponse<NotetakerLeaveResponse>> {
  console.log('\n=== Leaving Notetaker ===');
...
async function main(): Promise<void> {
  try {
    ...
    if (notetaker.data.state === 'media_available') {
      ...
      console.log(
        `You can check back later when the state is 'media_available'.`
      );
    }
