console.log(
  "Hiya! Welcome to the Node SDK test program. I need your help to make sure we didn't break the SDK."
);
console.log('');
...
nylas.deltas.generateCursor(timestampMs, function(error, cursor) {
  ...
  color_display(
    'Started delta stream --- try sending a new message and see if it appears here. You may have to use Ctrl-C to quit though.'
  );
  stream
    .on('delta', function(delta) {
...
nylas.calendars.list({}).then(function(calendars) {
  ...
  nylas.events.list({}).then(function(events) {
    ev = nylas.events.build({
      title: 'Test node event',
      start: Math.floor(Date.now() / 1000),
      end: Math.floor((Date.now() + 20000) / 1000),
    });
