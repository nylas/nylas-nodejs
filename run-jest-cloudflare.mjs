...
// Create a Jest configuration for Cloudflare Workers
...
// Create a Cloudflare Workers test runner
async function createCloudflareTestRunner() {
  const workerCode = `// Cloudflare Workers Jest Test Runner
...
export default {
  async fetch(request, env) {
    ...
    if (url.pathname === '/test') {
      ...
      return new Response(JSON.stringify({
        ...
        results: results,
        environment: 'cloudflare-workers-jest',
    ...
    return new Response(JSON.stringify({
      message: 'Nylas SDK Jest Test Runner for Cloudflare Workers',
      endpoints: {
  ...
  await writeFile(join(__dirname, 'cloudflare-test-runner', 'jest-runner.mjs'), workerCode);
  console.log('✅ Created Jest test runner for Cloudflare Workers');
}

async function runJestTestsInCloudflare() {
  const workerDir = join(__dirname, 'cloudflare-test-runner');
  
  console.log('📦 Using Jest test runner worker:', workerDir);
  
  // Create the Jest configuration and test runner
  await createJestConfig();
  await createCloudflareTestRunner();
  
  // Start Wrangler dev server
  console.log('🚀 Starting Wrangler dev server...');
  
  const wranglerProcess = spawn('wrangler', ['dev', '--local', '--port', '8798'], {
    cwd: workerDir,
    stdio: 'pipe'
  });
  ...
  try {
    // Run the tests
    console.log('🧪 Running Jest test suite in Cloudflare Workers...');
    
    const response = await fetch('http://localhost:8798/test');
    ...
    if (result.status === 'PASS') {
      console.log('\n🎉 All Jest tests passed in Cloudflare Workers environment!');
      ...
      console.log('\n❌ Some Jest tests failed in Cloudflare Workers environment');
...
async function main() {
  ...
  const success = await runJestTestsInCloudflare();
  process.exit(success ? 0 : 1);
}

// Run the tests
main().catch(error => {
  console.error('💥 Jest test runner failed:', error);
  process.exit(1);
});