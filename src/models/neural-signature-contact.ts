...
export default class NeuralSignatureContact extends RestfulModel {
    ...
    toContactObject(): Contact {
        ...
        if(this.emails) {
            ...
            this.emails.forEach(email => contactEmails.push(new EmailAddress(this.connection, {type: "personal", email: email})))
            contact.emailAddresses = contactEmails;
        }
        if(this.phoneNumbers) {
            const contactNumbers: PhoneNumber[] = [];
            this.phoneNumbers.forEach(number => contactNumbers.push(new PhoneNumber(this.connection, {type: "mobile", number: number})));
        ...
        if(this.links) {
            ...
            this.links.forEach(link => {
                if(link['url']) {
                    webPages.push(new WebPage(this.connection, {type: "homepage", url: link['url']}));
                }
            })
