...
export default class Neural extends RestfulModel {
    sentimentAnalysisMessage(messageId: string): Promise<NeuralSentimentAnalysis> {
    ...
    extractSignature(messageId: string, parseContact?: boolean, options?: NeuralMessageOptions): Promise<NeuralSignatureExtraction> {
        ...
        return this._request(path, body).then(json => {
            ...
            if(parseContact !== false) {
                signature.contacts = new NeuralSignatureContact(this.connection, signature.contacts);
            }
            return signature;
        });
    }
    ...
    categorize(messageId: string): Promise<NeuralCategorizer> {
        ...
        return this._request(path, body).then(json => {
            ...
            category.categorizer = new Categorize(this.connection, category.categorizer);
            return category;
        })
    }

    cleanConversation(messageId: string, options?: NeuralMessageOptions): Promise<NeuralCleanConversation> {
        let body: { [key: string]: any; } = {'message_id': [messageId]};
